...
export default async function ResumeDetailPage({ params }: { params: Promise<{ id: string }> }) {
	...
	const [found] = await db.select().from(resume).where(eq(resume.id, id));
					...
					<div className="rounded-xl border border-border bg-card p-5">
						...
						<div className="flex flex-wrap gap-1.5">
							{parsed.missingConcepts.slice(0, 12).map((concept) => (
								<span
									key={concept}
									className="rounded-md border border-amber-500/20 bg-amber-500/5 px-2 py-0.5 text-xs text-amber-600 dark:text-amber-400"
								>
									{concept}
								</span>
							))}
