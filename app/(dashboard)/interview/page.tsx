...
export default async function InterviewListPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Interview Prep" description={`${interviews.length} question set${interviews.length !== 1 ? 's' : ''} generated`} />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">
                      {i.job?.title ?? 'Interview Set'} {i.job?.company && <span className="text-muted-foreground font-normal text-xs">at {i.job.company}</span>}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {questions.length} questions · {technicalCount} technical · {behavioralCount} behavioral · {new Date(i.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                <ArrowRight size={14} className="text-muted-foreground/40 group-hover:text-muted-foreground transition-colors shrink-0" />
              </Link>
            );
          })}
        </div>
