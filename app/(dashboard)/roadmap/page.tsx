...
export default async function RoadmapListPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Learning Roadmaps" description={`${roadmaps.length} roadmap${roadmaps.length !== 1 ? 's' : ''} created`} />
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {r.job?.title ?? 'Roadmap'} {r.job?.company && <span className="text-muted-foreground font-normal text-xs">at {r.job.company}</span>}
                  </p>
