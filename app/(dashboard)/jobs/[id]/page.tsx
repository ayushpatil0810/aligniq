  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-3xl">
      <div className="flex items-start justify-between gap-4">
        ...
        />
        <Link href="/resumes">
          <Button size="sm" className="shrink-0 gap-2 bg-primary text-primary-foreground hover:bg-primary/90" id="btn-analyze-this-job">
      ...
      <div className="flex gap-2">
        <span className={cn('rounded-md border px-2.5 py-1 text-xs font-medium capitalize', levelColors[job.level] ?? 'bg-muted text-muted-foreground border-border')}>
          {job.level}
        </span>
          ...
          {requirements.technicalSkills && (
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              {requirements.experienceYears !== undefined && (
                <p className="mt-3 text-xs text-muted-foreground">{requirements.experienceYears}+ years of experience required</p>
              )}
            </div>
          )}
            ...
            <div className="rounded-xl border border-border bg-card p-5 sm:col-span-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {requirements.niceToHave.map((s) => (
                  <span key={s} className="rounded-md border border-dashed border-border bg-muted/50 px-2 py-0.5 text-xs text-muted-foreground">
                    {s}
                  </span>
                ))}
              </div>
