  ...
  const analysesWithDetails = await Promise.all(
    analyses.map(async (a) => {
      const [[job], [resumeRecord]] = await Promise.all([
        db.select({ title: jobDescription.title, company: jobDescription.company }).from(jobDescription).where(eq(jobDescription.id, a.jobId)),
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">
                      ...
                      {a.job?.company && <span className="ml-1.5 text-muted-foreground font-normal text-xs">at {a.job.company}</span>}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {a.resumeRecord?.fileName ?? 'Unknown resume'} · {new Date(a.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {a.matchScore !== null && (
                    <span className={cn('text-sm font-semibold tabular-nums',
                      (a.matchScore ?? 0) >= 75 ? 'text-emerald-500' : (a.matchScore ?? 0) >= 50 ? 'text-amber-500' : 'text-red-500',
                    )}>
                      {a.matchScore}%
                    </span>
                  )}
