...
export default async function AnalysisPage({
  ...
  const [[resumeRecord], [job], [existingRoadmap], [existingInterview]] = await Promise.all([
    ...
    db.select({ title: jobDescription.title, company: jobDescription.company, level: jobDescription.level })
      .from(jobDescription).where(eq(jobDescription.id, analysis.jobId)),
    db.select({ id: roadmap.id }).from(roadmap).where(eq(roadmap.analysisId, id)),
    db.select({ id: interviewQuestion.id }).from(interviewQuestion).where(eq(interviewQuestion.analysisId, id)),
  ]);
