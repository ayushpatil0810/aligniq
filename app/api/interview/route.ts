...
export async function POST(req: NextRequest) {
  try {
    ...
    const [resumeRecord] = await db
      .select()
      .from(resume)
      .where(eq(resume.id, analysis.resumeId));
    ...
    const interviewSet = await generateInterviewQuestions({
      parsedResume: resumeRecord.parsedData as Parameters<typeof generateInterviewQuestions>[0]['parsedResume'],
      analysis: {
        matchScore: analysis.matchScore ?? 0,
        skillsMatched: (analysis.skillsMatched as Parameters<typeof generateInterviewQuestions>[0]['analysis']['skillsMatched']) ?? [],
        skillsMissing: (analysis.skillsMissing as Parameters<typeof generateInterviewQuestions>[0]['analysis']['skillsMissing']) ?? [],
