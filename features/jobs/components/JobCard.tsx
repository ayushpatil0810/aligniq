  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mt-1">
        <Badge variant="outline" className="bg-transparent text-[10px] tracking-wider uppercase font-medium">
          {job.level}
        </Badge>
        <Badge variant="secondary" className="text-foreground text-[10px] tracking-wider uppercase font-medium">
          {job.category}
        </Badge>
      </div>
      ...
      {requirements?.technicalSkills && (
        <div className="mt-2 flex flex-wrap items-center gap-1.5 pt-4 border-t border-border/50">
          {requirements.technicalSkills.slice(0, 4).map((skill, i) => (
            <span key={skill} className="text-xs text-muted-foreground font-medium flex items-center">
              {skill}
          ...
          {requirements.technicalSkills.length > 4 && (
            <Badge variant="outline" className="text-[9px] uppercase tracking-wider text-muted-foreground ml-1 bg-transparent">
