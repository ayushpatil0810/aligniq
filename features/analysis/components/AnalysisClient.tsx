...
const readinessConfig: Record<string, { label: string; color: string; bg: string }> = {
  'strong': { label: 'Strong Candidate', color: 'text-emerald-500', bg: 'bg-emerald-500/10 border-emerald-500/20' },
  'ready': { label: 'Ready', color: 'text-blue-500', bg: 'bg-blue-500/10 border-blue-500/20' },
  'developing': { label: 'Developing', color: 'text-amber-500', bg: 'bg-amber-500/10 border-amber-500/20' },
  'not-ready': { label: 'Not Ready Yet', color: 'text-red-500', bg: 'bg-red-500/10 border-red-500/20' },
};
...
export function AnalysisClient({ analysis, hasRoadmap, hasInterview, roadmapId, interviewId }: Props) {
  const router = useRouter();
  const [isGeneratingRoadmap, setIsGeneratingRoadmap] = useState(false);
  const [isGeneratingInterview, setIsGeneratingInterview] = useState(false);

  const skillsMatched = (analysis.skillsMatched as Array<{ skill: string; proficiencyLevel: string; relevance: string }>) ?? [];
  const skillsMissing = (analysis.skillsMissing as Array<{ skill: string; priority: string; learningTimeWeeks: number; reason: string }>) ?? [];
  ...
  // Radar chart data
  const radarData = [
    { subject: 'Technical Match', value: Math.min(100, (skillsMatched.filter(s => s.relevance === 'core').length / Math.max(1, skillsMatched.length + skillsMissing.filter(s => s.priority === 'high').length)) * 100) },
    { subject: 'Skill Breadth', value: Math.min(100, skillsMatched.length * 8) },
    { subject: 'Gap Severity', value: Math.max(0, 100 - skillsMissing.filter(s => s.priority === 'high').length * 20) },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card p-6">
          ...
          {readiness && (
            <span className={cn('mt-2 rounded-full border px-3 py-1 text-xs font-medium', readiness.bg, readiness.color)}>
              {readiness.label}
            </span>
          )}
        </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        <div className="space-y-2">
          {skillsMissing.map((s, i) => {
            ...
            return (
              <div key={i} className="flex items-center justify-between rounded-lg border border-border bg-background/50 px-4 py-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className={cn('rounded-md border px-2 py-0.5 text-[10px] font-medium', pCfg.color)}>
                    {pCfg.label}
                  </span>
          ...
          {skillsMissing.length === 0 && (
            <p className="text-center py-4 text-sm text-muted-foreground">No significant skill gaps detected! 🎉</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="mb-3 text-sm font-semibold text-foreground">
          Matched Skills
          <span className="ml-2 text-xs font-normal text-muted-foreground">{skillsMatched.length} skills</span>
        </h3>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-5">
          ...
          {hasRoadmap && roadmapId ? (
            <Button variant="outline" size="sm" className="gap-2 self-start" onClick={() => router.push(`/roadmap/${roadmapId}`)} id="btn-view-roadmap">
              View Roadmap <ArrowRight size={13} />
            </Button>
          ) : (
            <Button size="sm" className="gap-2 self-start bg-primary text-primary-foreground hover:bg-primary/90" onClick={generateRoadmap} disabled={isGeneratingRoadmap} id="btn-generate-roadmap">
              {isGeneratingRoadmap ? (
                <><div className="h-3 w-3 animate-spin rounded-full border-2 border-primary-foreground border-t-transparent" /> Generating...</>
              ) : (
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-5">
          ...
          {hasInterview && interviewId ? (
            <Button variant="outline" size="sm" className="gap-2 self-start" onClick={() => router.push(`/interview/${interviewId}`)} id="btn-view-interview">
              View Questions <ArrowRight size={13} />
            </Button>
          ) : (
            <Button size="sm" className="gap-2 self-start bg-primary text-primary-foreground hover:bg-primary/90" onClick={generateInterview} disabled={isGeneratingInterview} id="btn-generate-interview">
