...
function ResumeCard({ resume: r }: { resume: Resume }) {
	...
	const parsedData = r.parsedData as {
				...
				<div className="flex-1 min-w-0">
					...
					<div className="mt-2 flex items-center flex-wrap gap-2">
						<Badge
							...
							className={cn(
								'gap-1.5 font-medium',
								r.status === 'done' ? 'bg-foreground text-background hover:bg-foreground/90' : ''
							)}
						>
						...
						{parsedData?.experienceLevel && (
							<Badge
								variant="outline"
								className="text-muted-foreground uppercase text-[10px] tracking-wider bg-transparent"
							>
								{parsedData.experienceLevel}
							</Badge>
						)}
			...
			<div className="flex items-center gap-3 shrink-0 mt-4 sm:mt-0 bg-background/50 p-1.5 rounded-lg border">
				...
				<Link href={`/resumes/${r.id}`}>
					<Button
						size="sm"
						variant="ghost"
						className="h-8 gap-2 hover:bg-accent hover:text-foreground"
					>
